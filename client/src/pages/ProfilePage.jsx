...
const ProfilePage = () => {
  ...
  const handleSubmit = async (e)=> {
    e.preventDefault();
    if(!selectedImage){
      const response = await updateProfile({fullName: name, bio});
      if(response?.success){
        navigate('/')
      }
    return;
  ...
  reader.onload = async ()=>{
    ...
    const response = await updateProfile({profilePic: base64Image, fullName: name, bio});
    if(response?.success){
      navigate('/');
    }
  }
    ...
    <div className='min-h-screen bg-cover bg-no-repeat flex items-center justify-center'>
     <div className='w-5/6 max-w-2xl backdrop-blur-2xl text-gray-300 border-2 border-gray-600 flex items-center justify-between max-sm:flex-col-reverse rounded-lg'>
      <form onSubmit={handleSubmit} className='flex flex-col gap-5 p-10 flex-1'>
        ...
        <textarea onChange={(e)=>setBio(e.target.value)} value={bio} placeholder='Write your bio...' required className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-violet-500' rows={4} />
